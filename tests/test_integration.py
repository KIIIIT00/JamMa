"""
AS-Mamba Integration and End-to-End Tests
...
def test_as_mamba_full_forward():
    ...
    try:
        ...
        data = {
            ...
            # Images
            'imagec_0': torch.randn(batch_size, 3, H, W),
            'imagec_1': torch.randn(batch_size, 3, H, W),
            
            # Features at 1/8 resolution (from backbone)
            'feat_8_0': torch.randn(batch_size, 160, H_c, W_c),  # ConvNeXtV2-nano
            'feat_8_1': torch.randn(batch_size, 160, H_c, W_c),
            
            # Features at 1/4 resolution (for fine matching)
            'feat_4_0': torch.randn(batch_size, 80, H_c * 2, W_c * 2),
            'feat_4_1': torch.randn(batch_size, 80, H_c * 2, W_c * 2),
            
            # Grid coordinates
            'grid_8': torch.stack(torch.meshgrid(
                torch.arange(H_c), torch.arange(W_c), indexing='ij'
            ), dim=-1).float().view(1, -1, 2).expand(batch_size, -1, -1),
        }
...
def test_loss_computation():
    ...
    try:
        ...
        data = {
            ...
            'predict_flow': [(
                torch.randn(2, batch_size, H_c, W_c, 4),  # flows_0to1
                torch.randn(2, batch_size, H_c, W_c, 4),  # flows_1to0
            )],
            
            # Ground truth for flow
            'spv_b_ids': torch.randint(0, batch_size, (n_matches,)),
            'spv_i_ids': torch.randint(0, H_c * W_c, (n_matches,)),
            'spv_j_ids': torch.randint(0, H_c * W_c, (n_matches,)),
            'hw0_c': (H_c, W_c),
            'hw1_c': (H_c, W_c),
            
            # Coarse matching
            'conf_matrix': torch.rand(batch_size, H_c * W_c, H_c * W_c),
            'conf_matrix_gt': torch.zeros(batch_size, H_c * W_c, H_c * W_c),
            
            # Fine matching
            'expec_f': torch.randn(n_matches, 3),  # with std
            'expec_f_gt': torch.randn(n_matches, 2),
        }
        
        # Add some ground truth matches
        for i in range(n_matches):
            b = data['spv_b_ids'][i]
            i_idx = data['spv_i_ids'][i]
            j_idx = data['spv_j_ids'][i]
...
def test_gradient_flow():
    ...
    try:
        ...
        data = {
            ...
            'h_8': H,
            'w_8': W,
            'feat_8_0': torch.randn(batch_size, 64, H * W, requires_grad=True),
            'feat_8_1': torch.randn(batch_size, 64, H * W, requires_grad=True),
        }
...
def test_multiblock_flow_collection():
    ...
    try:
        ...
        config = {
            ...
            'resolution': [8, 2],
            'as_mamba': {
                'n_blocks': 3,  # Test multiple blocks
                'd_geom': 32,
                'use_kan_flow': False,
                'global_depth': 1,
                'local_depth': 1,
        ...
        data = {
            ...
            'w_8': W_c,
            'c': 128,
            'imagec_0': torch.randn(batch_size, 3, 128, 160),
            'imagec_1': torch.randn(batch_size, 3, 128, 160),
            'feat_8_0': torch.randn(batch_size, 160, H_c, W_c),
            'feat_8_1': torch.randn(batch_size, 160, H_c, W_c),
            'feat_4_0': torch.randn(batch_size, 80, H_c * 2, W_c * 2),
            'feat_4_1': torch.randn(batch_size, 80, H_c * 2, W_c * 2),
            'grid_8': torch.zeros(batch_size, H_c * W_c, 2),
        }
    ...
    try:
        ...
        def run_forward():
            data = {
                ...
                'h_8': H,
                'w_8': W,
                'feat_8_0': torch.randn(batch_size, 256, H * W, device=device),
                'feat_8_1': torch.randn(batch_size, 256, H * W, device=device),
            }
            with torch.no_grad():
                output = block(data)
            return output
        ...
        if device == 'cuda':
            ...
            if current_memory < peak_memory * 0.5:  # Reasonable threshold
    ...
    try:
        ...
        for bs in batch_sizes:
            data = {
                ...
                'h_8': H,
                'w_8': W,
                'feat_8_0': torch.randn(bs, 128, H * W),
                'feat_8_1': torch.randn(bs, 128, H * W),
            }
...
def test_adaptive_span_diversity():
    ...
    try:
        ...
        # High variance features should lead to diverse spans
        data = {
            'bs': batch_size,
            'h_8': H,
            'w_8': W,
            'feat_8_0': torch.randn(batch_size, 128, H * W) * 2.0,  # High variance
            'feat_8_1': torch.randn(batch_size, 128, H * W) * 2.0,
        }
